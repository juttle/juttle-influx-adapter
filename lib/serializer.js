...
Serializer.prototype.toInflux = function(point) {
    ...
    var measurement = point[this.measurementField];
    var timestamp   = point.time ? point.time.unixms() : null;

    var point = _.omit(point, this.measurementField, 'time');
    ...
    return _.compact([key.join(","), val.join(","), timestamp]).join(" ");
};
