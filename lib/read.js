...
var Read = Juttle.proc.source.extend({
    ...
    initialize: function(options, params, pname, location, program, juttle) {
        var allowed_options = ['raw', 'db', 'names', 'offset', 'limit', 'fields', 'nameField', 'from', 'to'];
        ...
        this.serializer = new Serializer(_.pick(options, 'nameField'));
        ...
        this.queryOptions = _.defaults(
            _.pick(options, 'names', 'offset', 'limit', 'fields', 'from', 'to'),
            {
                limit: 1000,
            }
        );
